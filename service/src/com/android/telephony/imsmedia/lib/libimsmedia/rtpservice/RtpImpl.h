...
class RtpImpl : public IRtpAppInterface
{
    ...
    public:
        ...
        eRtp_Bool rtpSsrcCollisionInd(IN RtpDt_Int32 uiOldSsrc,
        ...
        eRtp_Bool rtpMemberLeaveInd(IN eRTP_LEAVE_REASON eLeaveReason,
                                      IN RtpDt_Int32 uiSsrc);

        eRtp_Bool rtcpPacketSendInd(IN RtpBuffer* pobjRtcpPkt,
                                      IN RtpSession *pobjRtpSession);

        eRtp_Bool rtcpAppPayloadReqInd(OUT RtpDt_UInt16 &usSubType,
                                     OUT RtpDt_UInt32& uiName,
                                     OUT RtpBuffer* pobjPayload);

        eRtp_Bool getRtpHdrExtInfo(OUT RtpBuffer* pobjExtHdrInfo);

        eRtp_Bool deleteRcvrInfo(IN RtpDt_UInt32 uiRemoteSsrc,
                               IN RtpBuffer *pobjDestAddr,
                               IN RtpDt_UInt16 usRemotePort);

        eRtp_Bool rtcpTimerHdlErrorInd(IN eRTP_STATUS_CODE eStatus);


        RtpDt_Void* RtpStartTimer(IN RtpDt_UInt32 uiDuration,
                                    IN eRtp_Bool bRepeat,
                                    IN RTPCB_TIMERHANDLER pfnTimerCb,
                                    IN RtpDt_Void *pvData);

        eRtp_Bool RtpStopTimer(IN RtpDt_Void* pTimerId,
                                  OUT RtpDt_Void** pUserData);
};
