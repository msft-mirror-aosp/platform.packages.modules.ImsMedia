...
void AudioJitterBuffer::SetJitterBufferSize(uint32_t nInit, uint32_t nMin, uint32_t nMax) {
    ...
    if (nInit > 0) {
        if (nInit < mMinJitterBufferSize) nInit = mMinJitterBufferSize;
        if (nInit > mMaxJitterBufferSize) nInit = mMaxJitterBufferSize;
...
void AudioJitterBuffer::SetJitterOptions(uint32_t nReduceTH, uint32_t nStepSize, double zValue,
...
void AudioJitterBuffer::Add(ImsMediaSubType subtype, uint8_t* pbBuffer, uint32_t nBufferSize,
    ...
    if (mBufferImprovement == false && mCanNotGetCount > 500) {
        ...
        if ((mCanNotGetCount > mMaxJitterBufferSize)
            && (mCanNotGetCount > mMinJitterBufferSize)) {
            IMLOGD0("[Add] Refreshed");
            //CollectJitterStatus(RTP_PACKET_STATUS_ANAL_JITTER_RESET, nSeqNum);
            Reset();
        }
    ...
    if (mBufferIgnoreSIDPacket == false) {
        ...
        if ((mBaseTS == 0 && mBaseAT == 0) || (mNeedToUpdateBasePacket == true)) {
            ...
            if (currEntry.nTimestamp - mBaseTS > currEntry.nInputTime - mBaseAT) {
                ...
                mBaseAT = currEntry.nInputTime;
                mJitterAnalyser.BasePacketChange(mBaseTS, mBaseAT);
            } else { // compensation case
                uint32_t temp = currEntry.nInputTime;
                IMLOGD_PACKET2(IM_PACKET_LOG_JITTER, "Before compensation[%u], seqNum[%d]",
                    temp, currEntry.nSeqNum);
        ...
        if ((mBaseTS == 0 && mBaseAT == 0) || (mNeedToUpdateBasePacket == true)) {
            ...
            if (currEntry.nTimestamp - mBaseTS > currEntry.nInputTime - mBaseAT) {
                ...
                IMLOGD_PACKET2(IM_PACKET_LOG_JITTER,
                    "Before compensation[%u], seqNum[%d]",temp, currEntry.nSeqNum);
                currEntry.nInputTime = mBaseAT + (currEntry.nTimestamp - mBaseTS);
                IMLOGD_PACKET2(IM_PACKET_LOG_JITTER,
                    "After compensation[%u], delay[%u]", currEntry.nInputTime,
                    temp - currEntry.nInputTime);
            }
        }
    ...
    IMLOGD_PACKET7(IM_PACKET_LOG_JITTER,
        "[Add] seq[%d], bMark[%d], nTS[%d], nbuffer[%d] subType[%d] QueueSize[%d] ninputTime[%u] ",
        nSeqNum, bMark, nTimestamp, nBufferSize, subtype,
        mDataQueue.GetCount()+1, currEntry.nInputTime);
    ...
    IMLOGD_PACKET1(IM_PACKET_LOG_JITTER,
        "[Add] Frames with Duplicated SeqNum Count [%d]",
        nMaxBundledCount);

    if (mBufferImprovement == false) {
        while (mDataQueue.GetCount() > mMaxJitterBufferSize +
            mMinJitterBufferSize + nMaxBundledCount - 1) {
            //CollectJitterStatus(RTP_PACKET_STATUS_LATE);
            mDataQueue.Delete();
            mDeleteCount++;
            IMLOGD_PACKET0(IM_PACKET_LOG_JITTER,
                "[Add] Delete - !mIsReceivedFirst");
        }
    } else {
...
bool AudioJitterBuffer::Get(ImsMediaSubType* psubtype, uint8_t** ppData,
    ...
    if (mBufferImprovement == false) {
        if ((mWaiting == false && bWait == true) || mUpdateJitterBufferSize > 2*50) {
            ...
            if (mUpdateJitterBufferSize > 2*50) {  // 2s
        ...
        if ((mWaiting == false && bWait == true)
            || mUpdateJitterBufferSize > 500) {
    ...
    if (bWait) {
        if (mDataQueue.Get(&pEntry)) {
            IMLOGD_PACKET5(IM_PACKET_LOG_JITTER,
                ...
                pEntry->nTimestamp, mBaseTS, mBaseAT);
        }

        if (mDataQueue.GetCount() == 0 || ((mDataQueue.Get(&pEntry)
            && (ImsMediaTimer::GetTimeInMilliSeconds() - pEntry->nInputTime) <
    ...
    while (mDataQueue.GetCount() > 0) {
        ...
        if (mDeleteCount > 4 && mDataQueue.GetCount() < mCurrJitterBufferSize+1) {
            ...
            if (mBufferImprovement == false) {
                ...
                if (mDataQueue.GetCount() > nTempBuferSize) {
                    ...
                    mCurrPlayingTS = pEntry->nTimestamp -
                        (nTempBuferSize-mDataQueue.GetCount()) * 20;
                    mCurrPlayingSeq = 0;
                }
            } else {
        ...
        if (USHORT_TS_ROUND_COMPARE(pEntry->nTimestamp, mCurrPlayingTS) == false) {
            ...
            mDataQueue.Delete();
            IMLOGD_PACKET0(IM_PACKET_LOG_JITTER,
                "[Get] Delete - pEntry->nTimestamp < mCurrPlayingTS)");
        } else {
            ...
            if ((timediff > 0) && (timediff < 20)) {
                IMLOGD2("[Get] Re sync2 - PacketTS[%d], PlayTS[%d]",
        ...
        IMLOGD4("[Get]  D [ %d / %u / %u / %d ]",
            pEntry->nSeqNum, pEntry->nTimestamp, mCurrPlayingTS, mDataQueue.GetCount());
    }
    ...
    if ((mCanNotGetCount > 10) && (mDataQueue.Get(&pEntry) == true)
        ...
        IMLOGD4("[Get] TS changing case [ %d / %u / %u / %d ]",
            pEntry->nSeqNum, pEntry->nTimestamp, mCurrPlayingTS, mDataQueue.GetCount());
        bForceToPlay = true;
    }
    ...
    if (mDataQueue.Get(&pEntry)) {
        ...
        IMLOGD_PACKET2(IM_PACKET_LOG_JITTER,
            "[Get] Determin - nDataQueue[%d], curTS[%d]", 0, mCurrPlayingTS);
    }
    ...
    if (mDataQueue.Get(&pEntry) == true && pEntry != NULL
        ...
        IMLOGD_PACKET7(IM_PACKET_LOG_JITTER,
            "[Get] OK - dtx mode[%d], curTS[%d], seq[%d], Mark[%d], TS[%d], buffer[%d], size[%d]",
        ...
        {
            if ((mCodecType == AUDIO_EVS) && (mDtxOn == false)
                && (mRedundancyOffSet > 0) && (mDataQueue.GetCount() > 0)) {
                // check partial redundancy packet
                bool ret = false;
                ret = CheckPartialRedundancyFrame(psubtype, ppData, pnDataSize,
                    pnTimestamp, pbMark, pnSeqNum, pnChecker);
        ...
        if (pbMark) *pbMark = false;
        if (pnSeqNum) *pnSeqNum = 0;
        if (mDataQueue.GetCount() == 0) mDataCount = 0;

        IMLOGD_PACKET2(IM_PACKET_LOG_JITTER,
            "[Get] fail - dtx mode[%d], curTS[%d]",
            mDtxOn, mCurrPlayingTS);
...
bool AudioJitterBuffer::CheckPartialRedundancyFrame(ImsMediaSubType* psubtype,
    uint8_t** ppData, uint32_t* pnDataSize, uint32_t* pnTimestamp, bool* pbMark,
    uint32_t* pnSeqNum, uint32_t* pnChecker)
{
    DataEntry* pEntry;
