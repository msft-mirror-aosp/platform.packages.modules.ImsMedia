...
IRtpSession* IRtpSession::GetInstance(ImsMediaType eMediaType,
...
void IRtpSession::ReleaseInstance(IRtpSession* pSession) {
    IMLOGD2("[ReleaseInstance] MediaType[%x], mRefCount[%d]",
        pSession->getMediaType(), pSession->getRefCounter());
...
IRtpSession::IRtpSession(ImsMediaType eMediaType,
    const RtpAddress local, const RtpAddress peer) {
...
bool IRtpSession::operator==(const IRtpSession &obj2){
    if (mMediaType == obj2.mMediaType
        && mLocalAddress == obj2.mLocalAddress
...
bool IRtpSession::isSameInstance(ImsMediaType eMediaType,
        const RtpAddress local, const RtpAddress peer) {
    if (mMediaType == eMediaType
        && mLocalAddress == local
        && mPeerAddress == peer) {
        return true;
    }
...
void IRtpSession::SetRtpEncoderListener(
    IRtpEncoderListener* pRtpEncoderListener) {
    std::lock_guard<std::mutex> guard(mutexEncoder);
    mRtpEncoderListener = pRtpEncoderListener;
}

void IRtpSession::SetRtpDecoderListener(
    IRtpDecoderListener* pRtpDecoderListener) {
    std::lock_guard<std::mutex> guard(mutexDecoder);
    mRtpDecoderListener = pRtpDecoderListener;
}

void IRtpSession::SetRtcpEncoderListener(
    IRtcpEncoderListener* pRtcpEncoderListener) {
    std::lock_guard<std::mutex> guard(mutexEncoder);
    mRtcpEncoderListener = pRtcpEncoderListener;
}

void IRtpSession::SetRtcpDecoderListener(
    IRtcpDecoderListener* pRtcpDecoderListener) {
    std::lock_guard<std::mutex> guard(mutexDecoder);
...
bool IRtpSession::SendRtpPacket(uint32_t nPayloadType, uint8_t* pData,
    ...
    mNumRtpDataToSend++;
    IMS_RtpSvc_SendRtpPacket(this, mRtpSessionId,
        (char*)pData, nDataSize, &stRtpPacketParam);
    return true;
}
...
bool IRtpSession::ProcRtpPacket(uint8_t* pData, uint32_t nDataSize) {
    ...
    *(unsigned int*)(pData+8) = ssrc;
    IMS_RtpSvc_ProcRtpPacket(this, mRtpSessionId, pData, nDataSize,
...
bool IRtpSession::ProcRtcpPacket(uint8_t* pData, uint32_t nDataSize) {
    ...
    mNumRtcpProcPacket++;
    IMS_RtpSvc_ProcRtcpPacket(this, mRtpSessionId, pData, nDataSize,
        mPeerAddress.ipAddress, mPeerAddress.port,
        &mLocalRtpSsrc);
    return true;
}
...
void IRtpSession::OnPeerInd(tRtpSvc_IndicationFromStack eIndType, void* pMsg) {
    ...
    switch (eIndType) {
         case RTPSVC_RECEIVE_RTP_IND:
            IMLOGD_PACKET1(IM_PACKET_LOG_RTP,
                "[OnPeerInd] RTP eIndType[%d]", eIndType);
            mNumRtpPacket++;
            if (mRtpDecoderListener) {
                ...
                if ((mEnableDTMF == false || mDtmfPayloadType != pstRtp->dwPayloadType)
                    ...
                    mRtpDecoderListener->OnMediaDataInd(pstRtp->pMsgBody, pstRtp->wMsgBodyLen,
                        pstRtp->dwTimestamp, pstRtp->bMbit, pstRtp->dwSeqNum, pstRtp->dwPayloadType,
                        nSSRC, pstRtp->bExtension, pstRtp->extensionData);
                }
            }
            break;
        ...
        case RTPSVC_SSRC_COLLISION_CHANGED_IND:
            IMLOGD_PACKET0(IM_PACKET_LOG_RTCP,
                "[OnPeerInd] RTPSVC_SSRC_COLLISION_CHANGED_IND]");
            break;
