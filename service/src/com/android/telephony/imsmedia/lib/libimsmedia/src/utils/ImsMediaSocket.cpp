...
ImsMediaSocket* ImsMediaSocket::GetInstance(
    ...
    for (auto&i:slistSocket) {
        if (strcmp(i->GetPeerIPAddress(), peerIpAddress) == 0
            && i->GetLocalPort() == localPort
...
ImsMediaSocket::~ImsMediaSocket() {
    IMLOGD_PACKET5(IM_PACKET_LOG_SOCKET,
        "[~ImsMediaSocket] %x, %s:%d %s:%d",
        this, mLocalIP, mLocalPort, mPeerIP, mPeerPort);
}
...
bool ImsMediaSocket::Open(int socketFd) {
    ...
    IMLOGD5("[Open] %s:%d, %s:%d, nRefCount[%d]", mLocalIP, mLocalPort,
        mPeerIP, mPeerPort, mRefCount);
...
uint32_t ImsMediaSocket::SendTo(uint8_t* pData, uint32_t nDataSize) {
    ...
    if (nLen < 0) {
        IMLOGE4("[ImsMediaSocket:SendTo] FAILED nLen(%d), nDataSize(%d) failed (%d, %s)",
    ...
    switch (nOption) {
        case SOCKET_OPT_IP_QOS:
            ...
            if (mLocalIPVersion == IPV4) {
                if (-1 == setsockopt(mSocketFd, IPPROTO_IP, IP_TOS, (void *)&mToS,
                    sizeof(uint32_t))) {
                    IMLOGE0("[SetSocketOpt] IP_TOS - IPv4");
                    return false;
                }
            } else {
                if (-1 == setsockopt(mSocketFd, IPPROTO_IPV6, IPV6_TCLASS, (void *)&mToS,
                    sizeof(uint32_t))) {
...
void ImsMediaSocket::StartSocketMonitor() {
    ...
    mbTerminateMonitor = false;
    IMLOGD_PACKET0(IM_PACKET_LOG_SOCKET,
       "[StartSocketMonitor] start monitor thread");
...
void ImsMediaSocket::StopSocketMonitor() {
    IMLOGD_PACKET0(IM_PACKET_LOG_SOCKET,
       "[StopSocketMonitor] stop monitor thread");
...
void ImsMediaSocket::SendNotify(void* pReadfds) {
    ...
    for (auto&i:slistRxSocket) {
        ...
        if (FD_ISSET(socketFD, (fd_set*)pReadfds)) {
            IMLOGD_PACKET1(IM_PACKET_LOG_SOCKET,
                "[SendNotify] send notify to listener %p", i->GetListener());
            if (i->GetListener() != NULL) i->GetListener()->OnReceiveEnabled();
        }
    }
}
